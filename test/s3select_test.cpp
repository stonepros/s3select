...
TEST(TestS3selectFunctions, csv_json_count)
{
  std::string input;
  std::string result;
  size_t size = 5;
  generate_csv(input, size);
  std::string js = convert_to_json(input.c_str(), input.size());
  std::string csv_query_1 = "select count(0) from s3object;";
  std::string s3select_result_1 = run_s3select(csv_query_1, input);

  const char* json_query_1 = convert_query(csv_query_1);
  run_json_query(json_query_1, js, result);

  ASSERT_EQ(result, s3select_result_1);
}

TEST(TestS3selectFunctions, sum_json)
{
  std::string input;
  std::string result;
  size_t size = 5;
  generate_csv(input, size);
  std::string js = convert_to_json(input.c_str(), input.size());
  std::string csv_query_1 = "select sum(int(_1)) from s3object;";
  std::string s3select_result_1 = run_s3select(csv_query_1, input);

  const char* json_query = convert_query(csv_query_1);
  std::cout<<json_query;
  run_json_query(json_query, js, result);
